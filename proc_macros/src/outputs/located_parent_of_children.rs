...
pub fn located_parent_of_children_toks(bounded_child_type: &TypeWithBounds, path_type: &TypePath, singular_snakecase: &LitStr, plural_snakecase: &LitStr, plural_titlecase: &LitStr) -> TokenStream {
    ...
    // Build final tokens:
    quote !{
        pub trait #located_parent_of_children<#lt,#child_generic_bounds Parent,ParentPath,JoinedPath:From<(ParentPath,#path_type)>>
        where Parent: #parent_of_children<#child_lifetime_generics #child_type_generics> + #lt, ParentPath: #lt + Clone {
            ...
            fn #get_children_owned(&self) -> impl IntoIterator<Item = #return_type> where #return_type: Clone { self.self_ref().obj.#get_children_owned() }

            fn #get_located_child(&self, path: #path_type) -> Result<#obj_at_path<#lt,#return_type,JoinedPath>,()> { 
        ...
        impl <#lt,Parent,ParentPath,JoinedPath,#child_generic_bounds> #located_parent_of_children<#lt,#child_lifetime_generics #child_type_generics Parent,ParentPath,JoinedPath,>
        for #obj_at_path<#lt,Parent,ParentPath> where Parent:#parent_of_children<#child_lifetime_generics #child_type_generics>, ParentPath: #lt + Clone, JoinedPath: From<(ParentPath,#path_type)> {
