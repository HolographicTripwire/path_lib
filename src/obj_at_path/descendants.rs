...
OldAtPath:'a + Path,
Joiner:'a + Path {
    fn valid_primitive_paths(&'a self) -> impl IntoIterator<Item = Joiner> { self._obj().valid_paths() }
    fn get_child(&'a self, path: &Joiner) -> Result<&'a Descendant,()> { self._obj().get_descendant(path) }
    ...
    fn get_descendant(&'a self) -> impl IntoIterator<Item = &'a Descendant> { 
        self.valid_primitive_paths()
            .into_iter()
            .map(|path| self.get_child(&path).expect("valid_primitive_paths returned an invalid path"))
    }
    fn get_located_descendants(&'a self) -> impl IntoIterator<Item = ObjAtPath<'a,Descendant,PathPair<OldAtPath,Joiner>>> {
        self.valid_primitive_paths()
            .into_iter()
            .map(|path| self.get_located_descendant(path).expect("valid_primitive_paths returned an invalid path"))
    }
}
